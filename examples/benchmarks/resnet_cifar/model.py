...
from normutil import LayerNorm, GRN
...
class ResNetCIFAR(nn.Module):
    ...
    class Block(nn.Module):
        ...
        def __init__(self, f_in: int, f_out: int, downsample: bool = False, mode: str = 'plain'):
            ...
            # self.act1 = nn.ReLU()
            self.act1 = aespa(num_pol=3, planes=f_out, norm = 'bnv3', lr_scaler=0.1)
            ...
            self.act2 = aespa(num_pol=3, planes=f_out, norm = 'bnv3', lr_scaler=0.1)
            # self.act2 = nn.ReLU()
            # self.bn2 = LayerNorm(f_out, eps=1e-6, data_format="channels_first")
            # self.bn3 = nn.BatchNorm2d(f_out, affine=False)

            # self.gamma = nn.Parameter(torch.ones((f_out)),requires_grad=True)
...
def build_composer_resnet_cifar(model_name: str,
    ...
    def weight_init(w: torch.nn.Module):
        ...
        if isinstance(w, nn.BatchNorm2d):
            if w.affine and not hasattr(w,'inner_act'):
...
#                                    bias=False)
#             # self.bn1 = nn.BatchNorm2d(f_out)
#             self.aespa1 = aespa(num_pol=3, planes=f_out)
#             self.conv2 = nn.Conv2d(f_out,
#                                    f_out,
#                                    kernel_size=3,
#                                    stride=1,
...
#                                    bias=False)
#             # self.bn2 = nn.BatchNorm2d(f_out)
#             self.aespa2 = aespa(num_pol=3, planes=f_out)
#             self.relu = nn.ReLU(inplace=True)
